...
//        echo "<br />startTime3=" . $dt2;
        ...
        $chkDup = Appointment::where('room_id', $request->roomId)
            ->whereRaw('(? between start_time and end_time OR ? between start_time and end_time)', [$dt, $dt2])
            ->get();
        if (count($chkDup) > 0) {
        ...
        $appointment->room_id = $request->roomId;
